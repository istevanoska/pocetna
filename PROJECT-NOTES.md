...
- 2026-09-13 — **Агрегатори на вести** moved to the top of the grid, above the first
  ...
  before the category loop, instead of sharing a `panel-pair` block with Цитат на денот.
  The quote panel stays after Банки и финансии and now stands alone; `.panel-pair` in
  `link-directory.scss` is left in place but unused. Deployed and verified live: the
  prerendered HTML lists Агрегатори на вести before Вести with all four links, and the
...
- 2026-09-13 — Unknown URLs now return a real 404 with `public/404.html` instead of
  200 plus the whole homepage. `SpaForwardController` serves the prerendered file for a
  route that has one and 404s everything else; after prerendering every real route has
  a file, so anything without one genuinely does not exist.
...
- `.panel-pair` in `link-directory.scss` has had no user since the aggregators panel
  moved out of that block — remove it if nothing else needs a break-inside wrapper.
